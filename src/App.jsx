...
const PDF_WEBHOOK_URL = import.meta.env.VITE_PDF_WEBHOOK_URL || "https://insidesuccess.app.n8n.cloud/webhook/87ad4f51-e02e-40e6-9e77-66886c2139fa";

// Compliance (future)
const COMPLIANCE_WEBHOOK_URL = import.meta.env.VITE_COMPLIANCE_WEBHOOK_URL || STORY_WEBHOOK_URL;

// Edit/Revamp Webhook (Workflow 3)
const EDIT_WEBHOOK_URL = import.meta.env.VITE_EDIT_WEBHOOK_URL || STORY_WEBHOOK_URL;
...
function App() {
  ...
  const handleGenerate = async ({ transcript, action }) => {
    ...
    console.log("Transcript preview (cleaned):", cleaned.substring(0, 100) + "...");
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorText.includes("not registered for POST")) {
          throw new Error('n8n Configuration Error: Webhook is set to GET. Change to POST.');
        }
        if (errorText.includes("Unused Respond to Webhook")) {
          throw new Error('n8n Configuration Error: Set Webhook "Respond" to "Using Respond to Webhook Node".');
        }
  ...
  const handleCreatePdf = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.clone().text().catch(() => "");
        throw new Error(`PDF creation failed (${response.status}): ${errorText.substring(0, 200)}`);
      }
  ...
  const handleRevise = async (instructions, actionOverride) => {
    ...
    try {
      ...
      try {
        ...
        revisedText = json.output || json.text || json.content || json.message || json.result || JSON.stringify(json);
      } catch {
